...
/**
 * Migration 006 — Tạo bảng recommendations
 *
 * Lưu kết quả phân tích và gợi ý từ hệ thống DSS (AHP + TOPSIS + LP).
 * Mỗi lần người dùng yêu cầu gợi ý → tạo 1 bản ghi mới trong bảng này.
 *
 * Cấu trúc dữ liệu thuật toán:
 *
 * criteria (JSON):
 *   [{ name: "protein", weight: 0.35, isBenefit: true }, ...]
 *
 * alternatives (JSON — kết quả TOPSIS):
 *   [{ foodId: 3, foodName: "Ức gà luộc", topsisScore: 0.82, rank: 1 }, ...]
 *
 * ahp_matrix (JSON — ma trận so sánh cặp n×n):
 *   [[1, 3, 5], [1/3, 1, 3], [1/5, 1/3, 1]]
 *
 * lp_solution (JSON — lượng ăn tối ưu LP):
 *   { "3": 200, "7": 150 }   // foodId → gram
 *
 * top_food_ids (JSON — top 5 gợi ý):
 *   [3, 7, 12, 5, 9]
 */
export class CreateRecommendationsTable implements MigrationInterface {
